...
class Jps():
    ...
    def pathfind(self, start, goal):
        self.start = astar.Node(start[0], start[1])
        self.goal = astar.Node(goal[0], goal[1])
        self.start.gscore = 0
    ...
    def jump(self, node, parent):
        ...
        if dr != 0 and dc != 0:
            if self.jump(astar.Node(r, c + dc), node):
                return node
            if self.jump(astar.Node(r + dr, c), node):
                return node
        # Recursive diagonal search
        if not self.wall([r, c + dc]) or not self.wall([r + dr, c]):
            return self.jump(astar.Node(r + dr, c + dc), node)
    ...
    def find_neighbors(self, node):
        if node.parent is not None:
            ...
            # Diagonal direction
            if dr != 0 and dc != 0:
                neighbors.append(astar.Node(r, c + dc))
                neighbors.append(astar.Node(r + dr, c))
                neighbors.append(astar.Node(r + dr, c + dc))
                # forced neighbors
                if self.wall([r, c - dc]):
                    neighbors.append(astar.Node(r + dr, c - dc))
                if self.wall([r - dr, c]):
            ...
            # Horizontal direction
                ...
                # forced neighbors
                if self.wall([r + 1, c]):
                    neighbors.append(astar.Node(r + 1, c + dc))
                if self.wall([r - 1, c]):
                    neighbors.append(astar.Node(r - 1, c + dc))
            # Vertical direction
            elif dr != 0:
